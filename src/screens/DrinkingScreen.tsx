...
const DrinkingScreen: React.FC<DrinkingScreenProps> = ({ onNext, onBack }) => {
    ...
    const options = [
        ...
        { id: 'frequent_drinker', label: 'Frequent Drinker' },
    ...
    return (
        <View style={styles.container}>
            ...
            >
                <View style={styles.mainContent}>
                    ...
                    <FadeUpView delay={300}>
                        <Text style={styles.subtitle}>Do you drink?</Text>
                    </FadeUpView>
                    ...
                    <FadeUpView delay={450} style={styles.optionsList}>
                        {options.map((opt) => (
                            <PremiumOptionRow
                                key={opt.id}
                                label={opt.label}
                                selected={drinking === opt.id}
                                onPress={() => setDrinking(opt.id)}
                            />
                        ))}
                    </FadeUpView>
                </View>
            ...
            >
                <TouchableOpacity
                    style={[styles.btnContinue, !drinking && styles.btnDisabled]}
                    onPress={handleNext}
                    activeOpacity={0.8}
                    disabled={!drinking}
                >
